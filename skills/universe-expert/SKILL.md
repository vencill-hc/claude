...
- `references/relabel_blast_radius.md` — exact affected-population counts for parser/taxonomy changes against gdulabs-production (local-classify + BQ-join roundtrip, stored-vs-new and rev-vs-rev modes, consent discipline, known traps). Was the standalone relabel-blast-radius skill until 2026-08-24.
...
- `references/consented_bq_probe.md` — the house pattern for one-off prod measurement scripts (dry-run gated, --execute + bytes cap, JSON-param inputs, per-run consent protocol). **Read this** before writing any ad-hoc script that queries `gdulabs-production`.
- `references/rails_repo_test_environment.md` — sibling repo: bring-up + test-run steps for `data-universe-rails` (asdf/docker PATH fixes, 127.0.0.2 alias, minimal compose service set, test-DB prepare, cleanup etiquette). **Read this** before running `bin/rails test` in that repo.
